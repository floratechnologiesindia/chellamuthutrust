export interface ThanksLetterData {
  donorName: string;
  amount: number;
  paymentMode?: string;
  paymentDate?: string;
...
export function generateThanksLetterHtml(data: ThanksLetterData): string {
  ...
  return `
    <div style="font-family: 'Times New Roman', Georgia, serif; max-width: 700px; margin: 0 auto; padding: 30px 40px; color: #222; background: #fff;">
      ...
      <p style="font-size: 14px; line-height: 1.8; margin: 10px 0;">
        ...
        on <strong>${paymentDate}</strong>${data.referenceNumber ? ` (Ref: ${data.referenceNumber})` : ''}
      ...
      <div style="font-size: 11px; color: #666; text-align: center; line-height: 1.6;">
        ...
        <p style="margin: 2px 0;">www.mschellamuthutrust.com</p>
        <p style="margin: 6px 0; font-style: italic; color: #1a237e;">
          Exempted u/s 80G(5) of Income Tax Act, 1961 | PAN: AAATM1310P
        </p>
      </div>
    </div>
  `;
